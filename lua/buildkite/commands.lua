...
function M.get_current_organization()
    local org_name, _ = config_module.get_current_organization()
    if org_name then
...
function M.debug_config()
    ...
    table.insert(lines,
        string.format("Effective branch: %s (%s)", project_info.branch or "unknown", project_info.branch_source))
...
function M.get_pipeline_info()
    ...
    vim.notify(string.format("Current pipeline: %s/%s", project_info.org_name, project_info.pipeline_slug),
        vim.log.levels.INFO)
end
...
function M.refresh_current_build()
    ...
    local ok, build, _ = pcall(function()
    ...
    if build then
        ...
        local status_icon = config.ui.icons[build.state] or "?"
        vim.notify(string.format("Build refreshed: %s %s #%s", status_icon, build.state, build.number),
            vim.log.levels.INFO)
    else
...
function M.show_branch_info()
    ...
    if project_info.branch then
        local status_msg = string.format("Current branch: %s (source: %s)", project_info.branch,
            project_info.branch_source)
    ...
    vim.api.nvim_create_user_command("Buildkite", function(opts)
        ...
        elseif cmd == "build" then
            ...
            elseif subcmd == "rebuild" then
                ...
                vim.notify("Usage: :Buildkite build [current|list|open|refresh|rebuild] [branch] [limit]",
                    vim.log.levels.INFO)
            end
