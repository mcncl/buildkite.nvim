...
function M.get_current_organization()
    ...
    local first_org_name, first_org_config = next(global_config.organizations)
    if first_org_name then
        global_config.current_organization = first_org_name
        M.save_global_config(global_config)
        return first_org_name, first_org_config
    end
